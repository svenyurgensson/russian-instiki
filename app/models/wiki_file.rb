  ...
  def validate
    if file_name
      if ! WikiFile.is_valid?(file_name)
        errors.add("file_name", "имя файла неправильное. Только латинские буквы, цифры, точки, подчеркивания, " +
           "dashes and spaces are accepted")
      elsif file_name == '.' or file_name == '..'
        errors.add("file_name", "не может быть '.' or '..'")
      end
    end
    ...
    errors.add("content", "пустой") if @content.nil? or @content.empty?
  end
