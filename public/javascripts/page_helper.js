...
//make the textarea resize to fit available space
  var f = $('MarkupHelp');
  if (f) {
    var hidebutton = new Element('input', {id:'hidebutton', type:'button', value: 'Скрыть помощь по форматированию'});
  ...
  $$('textarea#content').each( function(textarea)  {
    ...
    Event.observe(hidebutton, 'click', function(){
      if (f.visible()) {
        f.hide();
        hidebutton.writeAttribute({value: 'Показать помощь по форматированию'});
        updateSize(textarea, w, h)
      } else {
        f.show();
        hidebutton.writeAttribute({value: 'Скрыть помощь по форматированию'});
        updateSize(textarea, w, h)
      }
    });
